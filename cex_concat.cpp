                        ...
                        const Term & original_trans)
{
  ...
  { // load variable assignment
    ...
    if (fin.is_open()) { 
      ...
      int idx; std::string name; std::string val; int idx0; int idx1;
      while((fin>>idx)) {
        fin>>name; fin>>val; fin>>idx0 ; fin>>idx1;
        idx += offset - 1;
        auto index = name.find("@");
        auto name_ext = name.substr(0,index);
        Term var_term = ts.lookup(name_ext);
        auto sort = var_term->get_sort();
        assert(val.substr(0,2) == "#b" && val.length() > 2);
        Term val_term = ts.make_term(val.substr(2), sort, 2);
        auto val_extract = ts.make_term(smt::Op(smt::PrimOp::Extract, idx0, idx1), val_term);
        auto var_extract = ts.make_term(smt::Op(smt::PrimOp::Extract, idx0, idx1), var_term);
        coi_info.insert(coi_info.begin(),{idx, var_extract, val_extract});
      }
    } else
  ...
  for (int j = 0; j < bound; j ++) {
    auto new_trans = unroller_.at_time(ts.trans(), j);
    s->assert_formula(new_trans);
  }
  ...
  for (const auto & idx_var_val : coi_info) {
    ...
    const auto & var_timed = unroller_.at_time(var, idx);
    s->assert_formula(ts.make_term(Equal, var_timed, val));
